...
public class InputNumAState implements State {

    // 数値入力A状態クラスのインスタンス(シングルトン)
    private static InputNumAState inputNumAStateInstance = new InputNumAState();

    /**
     * コンストラクタ
     */
    private InputNumAState() {}

    /**
     * インスタンスのゲッター
     */
    public static State getInputNumAStateInstance() {
        return inputNumAStateInstance;
    }
    ...
    public void onNumBtn(Context context, String inputNum) {
        ...
        // 状態遷移(⇨InputNumAState)
        context.changeState(inputNumAStateInstance);
    }
    ...
    public void onDecimalPointBtn(Context context) {
        // 小数点をディスプレイに反映
        context.setValDisplay(Const.DECIMAL_POINT_STRING);

        // 状態遷移(⇨InputDecimalAState)
        context.changeState(InputDecimalAState.getInputDecimalAStateInstance());
    }
